...
const CommentMessages = () => {
  ...
  const {
    ...
    pending: pending2,
    error2,
  } = useFetch("http://blog-server-zeta.vercel.app/getComments/" + id);
  const {
    data: likes,
    pending: ped,
    error3,
  } = useFetch("http://blog-server-zeta.vercel.app/getlikes/" + id);
if(likes){
  // console.log(likes);
  // <h3>Loading....</h3>
}
  ...
  const handleLikeClick = async() => {
    ...
    const response= await fetch("http://blog-server-zeta.vercel.app/like", {
        method: "POST",
        headers: { "content-type": "application/json" },
